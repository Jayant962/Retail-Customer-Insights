...
SELECT campaign_type,SUM(conversions) AS Total_Conversion
FROM campaign
GROUP BY campaign_type
...
SELECT product_category AS Highest_Revenue_Category
FROM [transaction]
...
SELECT product_category AS Highest_Avg_Rating,AVG(rating) Avg_Rating
FROM review
GROUP BY product_category
